...
module.exports = (app) => {
  ...
  var notificationDB = app.notificationDB;
  ...
  notificationsRouter.get('/', function (req, res) {
    ...
    notificationDB.count(req.query).exec(function (error, count) {
      let query = notificationDB.find(req.query);
  ...
  notificationsRouter.get('/:id', function (req, res) {
    notificationDB.find({ id: req.params.id }).exec(function (error, notifications) {
